...
def load_data(columns):
    ...
    sql = "SELECT "+select_columns+" FROM var_radial_buffer_sky_exposure_temp WHERE june21 IS NOT NULL ORDER BY june21;"
...
regr = MLPRegressor(hidden_layer_sizes=(100,), activation='logistic', random_state=1, max_iter=500, verbose=True, solver='adam', learning_rate_init=0.001, batch_size=200)
...
scores = cross_validate(pipeline, X, y, cv=5, scoring=['neg_mean_absolute_error', 'neg_root_mean_squared_error', 'r2'])
